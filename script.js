...
async function loadMiniPrecios() {
  const { data, error } = await supabaseClient
    .from("minicasillas")
    .select("id, precio");
  ...
  data.forEach(row => {
    const el = document.querySelector(`.mini-cell:nth-child(${row.id})`);
    if (el) el.textContent = row.id + " - " + row.precio + "€";
  });
}
loadMiniPrecios();
...
// ==================== ADMIN SALDO =====================
function updateHUD() {
  const s = localStorage.getItem("saldo") || 0;
  document.getElementById("hud-saldo").textContent = s;
}
...
function moverJugador(pasos) {
  ...
  dibujarJugador();
  eventoCasilla(jugador.posicion); // 👈 NUEVO
}
function eventoCasilla(id) {
  const precioEl = document.getElementById("p" + id);
  if (!precioEl) return;

  const precio = parseInt(precioEl.textContent);

  if (!isNaN(precio)) {
    // Mostrar botón de COMPRA
    mostrarCompra(id, precio);
  }
}
function mostrarCompra(id, precio) {
  const box = document.getElementById("compra-box");
  const info = document.getElementById("compra-info");

  info.textContent = `Casilla ${id} - Precio: ${precio}€`;

  box.classList.remove("hidden");

  // --- BOTÓN COMPRAR ---
  document.getElementById("btnComprar").onclick = () => {
    if (jugador.saldo >= precio) {
      jugador.saldo -= precio;

      localStorage.setItem("saldo", jugador.saldo);
      document.getElementById("hud-saldo").textContent = jugador.saldo;

      alert("¡Has comprado la casilla " + id + "!");
...
function mostrarCompra(id, precio) {
  ...
  const box = document.getElementById("compraBox");
  const txtPrecio = document.getElementById("compraPrecio");
  const titulo = document.getElementById("compraTitulo");

  titulo.textContent = "Comprar casilla " + id;
  txtPrecio.textContent = "Precio: " + precio + "€";

  box.classList.remove("compra-oculto");

  // Guardamos temporalmente qué casilla estamos comprando
  box.dataset.casilla = id;
  box.dataset.precio = precio;
}

function ocultarCompra() {
  document.getElementById("compraBox").classList.add("compra-oculto");
}
document.getElementById("btnCancelarCompra").addEventListener("click", ocultarCompra);

document.getElementById("btnComprar").addEventListener("click", () => {
  const box = document.getElementById("compraBox");
  const id = parseInt(box.dataset.casilla);
  const precio = parseInt(box.dataset.precio);
...
document.getElementById("btnDados").addEventListener("click", tirarDados);
